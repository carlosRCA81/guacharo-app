/* SEGURIDAD CRCA - SISTEMA BLINDADO */

// Aquí defines tu clave secreta. Cámbiala por la que prefieras.
...
function checkAccess() {
    ...
    if (inputPass === CLAVE_MAESTRA) {
        // Efecto de entrada exitosa
        alert("ACCESO CONCEDIDO: BIENVENIDO AL ANALIZADOR CRCA");
        
        // Ocultar login y mostrar la App
        loginScreen.style.display = 'none';
        mainApp.style.display = 'block';
        
        // Una vez dentro, inicializamos la base de datos de animales
        inicializarSistema();
    } else {
        alert("ERROR: CLAVE INCORRECTA. ACCESO DENEGADO.");
        document.getElementById('access-key').value = ''; // Limpiar el campo
    }
}

// Permitir entrar presionando "Enter" en el teclado del móvil
document./* LÓGICA INTEGRAL ANALIZADOR CRCA - VERSIÓN PATRONES ACTUALIZADA */
...
('access-key').addEventListener('keypress', function (e) {
    if (e.key === 'Enter') {
        checkAccess();
    }
});
