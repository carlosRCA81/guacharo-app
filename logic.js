...
function generarPanelDiario() {
    ...
    horasSorteo.forEach(hora => {
        ...
        if (reg) { box.classList.add('jugado'); box.innerText = `${hora}\n(${reg.num})`; }
...
async function consultarFechaEspecifica() {
    ...
    cuerpo.innerHTML = '<tr><td colspan="5">Buscando...</td></tr>';
    try {
        const { data, error } = await _supabase.from('historial_sorteos').select('*').eq('fecha', fechaBusqueda);
        if (data) {
            cuerpo.innerHTML = '';
    ...
    } catch (err) { cuerpo.innerHTML = '<tr><td colspan="5">Error de red</td></tr>'; }
}
...
async function cargarHistorialRemoto() {
    ...
    } catch (err) { console.log("Error inicial"); }
}

function actualizarInterfaz() {
    actualizarTabla();
    generarPanelDiario();
    // (Aquí puedes añadir las demás funciones de balance y guacharo)
}
...
function actualizarTabla() {
    ...
    if(!cuerpo) return;
    cuerpo.innerHTML = '';
    [...historial].sort((a,b) => b.fecha.localeCompare(a.fecha) || horasSorteo.indexOf(b.hora) - horasSorteo.indexOf(a.hora)).forEach(r => {
        const isGuacharo = r.num === '75' ? 'class="row-guacharo"' : '';
        cuerpo.innerHTML += `<tr ${isGuacharo}><td>${r.fecha}</td><td>${r.hora}</td><td>${r.num}</td><td>${r.animal}</td><td>${r.tipo}</td></tr>`;
    });
}

function llenarSelectEstudio() { /* Tu lógica de select */ }
