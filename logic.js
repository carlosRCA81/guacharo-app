// ==========================================
// CONFIGURACIÓN SUPABASE - ANALIZADOR FINAL
// ==========================================
...
async function inicializarSistema() {
    ...
    if(fAnalisis) {
        fAnalisis.value = hoy;
        fAnalisis.addEventListener('change', () => {
            generarPanelDiario();
            actualizarInterfaz();
        });
    }
...
// ==========================================
// EL ALGORITMO (ESPEJO, ESCALERA, SEGUIDILLA)
// ==========================================
function ejecutarEstudioAlgoritmo() {
    ...
    // Sugerencia Top 3 Histórico
    const conteo = {};
    historial.forEach(r => conteo[r.num] = (conteo[r.num] || 0) + 1);
    const top3 = Object.entries(conteo).sort((a,b) => b[1] - a[1]).slice(0,3).map(e => e[0]);
    htmlAnalisis += `<div style="color:#fbbf24; margin-bottom:10px;">📊 TOP 3 SUGERIDOS: <strong>${top3.join(' - ')}</strong></div>`;
    ...
    if(sorteosHoy.length > 0) {
        ...
        if(parseInt(esp) <= 75) htmlAnalisis += `<span class="badge espejito">ESPEJO: ${esp}</span> `;
        
        // Escalera
        let esc = (parseInt(ult.num) + 1).toString().padStart(2, '0');
        if(parseInt(esc) <= 75) htmlAnalisis += `<span class="badge escalera">ESCALERA: ${esc}</span> `;
        
        // Seguidilla (Tipo de animal)
        if(sorteosHoy.length >= 2) {
            const penultimo = sorteosHoy[sorteosHoy.length - 2];
            if(ult.tipo === penultimo.tipo) htmlAnalisis += `<span class="badge seguidilla">SEGUIDILLA ${ult.tipo}</span>`;
        }
    }

    alertaDiv.innerHTML = htmlAnalisis || "Esperando datos del día...";
}
...
function generarPanelDiario() {
    ...
    horasSorteo.forEach(hora => {
        ...
        if (reg) {
            ...
            box.innerHTML = `<strong>${hora}</strong><br><span style="color:#fbbf24; font-weight:bold;">${reg.num}</span>`;
        } else {
            box.innerText = hora;
        }
...
async function registrarSorteo(num, animal, tipo, hora) {
    ...
    } catch (err) { console.error("Error nube"); }
}
...
function actualizarTabla(datos = null) {
    ...
    // Si no hay datos filtrados, mostramos los últimos 15 de toda la historia
    const lista = datos || [...historial].sort((a, b) => b.fecha.localeCompare(a.fecha) || horasSorteo.indexOf(b.hora) - horasSorteo.indexOf(a.hora)).slice(0, 15);
...
function registrarPorNumero() {
    ...
    if (v !== "0" && v !== "00" && v.length === 1) v = "0" + v;
    const ani = listaAnimales.find(a => a.n === v);
    if(ani) { registrarSorteo(ani.n, ani.a, ani.t, horaSeleccionadaActiva); input.value = ''; }
}
...
function generarGridBotones() {
    ...
    listaAnimales.forEach(a => {
        ...
        b.innerHTML = `<strong>${a.n}</strong><br><small>${a.a}</small>`;
        b.onclick = () => {
            if (!horaSeleccionadaActiva) return alert("Selecciona HORA");
