...
// Clasificación: E=Elemento (Agua, Tierra, Aire)
const listaAnimales = [
    ...
    {n:'05', a:'LEON', c:'NEGRO', s:'C', e:'Tierra'}, {n:'06', a:'RANA', c:'NEGRO', s:'F', e:'Agua'},
...
function actualizarJugadaSniper() {
    ...
    if (esAlertaCaliente) {
        ...
        aviso.innerText = "⚠️ ATENCIÓN: PRÓXIMA JUGADA EN LOS 2 SIGUIENTES SORTEOS";
        try { document.getElementById('snd-alerta').play(); } catch(e){}
    } else {
        let n = parseInt(ultimo.num);
        sugeridos = [(n + 1).toString().padStart(2, '0'), '11', '25'];
...
function generarGridBotones() {
    ...
    listaAnimales.forEach(a => {
        ...
        d.innerHTML = `<b>${a.n}</b><br>${a.a}<br><small>Sec ${a.s}</small>`;
...
function actualizarTabla() {
    ...
    historial.filter(r => r.fecha === f).sort((a,b) => horasSorteo.indexOf(a.hora) - horasSorteo.indexOf(b.hora)).forEach(r => {
        ...
        if(ani) {
            c.innerHTML += `<tr><td>${r.hora}</td><td>${r.num}</td><td>${r.animal}</td><td>${ani.s} / ${ani.e}</td><td><span class="badge ${ani.c === 'ROJO' ? 'bg-rojo' : ani.c === 'NEGRO' ? 'bg-negro' : 'bg-azul'}">${ani.c}</span></td></tr>`;
        }
    });
}
